...
export function TvAiAnalysis({ days, brandName, hasData }: TvAiAnalysisProps) {
  ...
  async function handleAnalyse() {
    if (!hasData) {
      toast.error('Upload a TV schedule first to run AI analysis.')
      return
    }
    setLoading(true)
    try {
      const res = await fetch('/api/tv/analyse', {
        method: 'POST',
        ...
        <Card className="border rounded-xl p-5 bg-card space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold">AI TV Analysis</span>
            </div>
            <button
