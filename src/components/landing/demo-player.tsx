...
/**
 * In-page unveil film. Every scene is a pure function of its local `t`, driven
 * by a single rAF clock, so the animation is deterministic and smooth — no
 * baked-in encode artefacts. It autoplays muted like a product showcase and
 * loops seamlessly: the loop seam lands on the opening title card, so the
 * Brand Health Index count-up never snaps back mid-sweep.
 */
...
export function DemoPlayer() {
  ...
  // Respect the viewer's reduced-motion preference: hold a static frame.
  useEffect(() => {
  ...
  // Pause the clock when the film scrolls out of view or the tab is hidden,
  // so it never burns CPU off-screen.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      last = now
      // Advance only while on-screen; a long background gap won't fast-forward.
      if (visibleRef.current && !document.hidden && dt < 0.5) {
  ...
  // Reduced motion: show the Brand Health Index at rest, fully drawn.
  const view = reduced ? { index: 1, chapter: TIMELINE[1], local: 1 } : chapterAt(time)
  const { index, chapter, local } = view
  ...
  function scrub(e: React.MouseEvent<HTMLDivElement>) {
    ...
    timeRef.current = Math.min(TOTAL, Math.max(0, ((e.clientX - rect.left) / rect.width) * TOTAL))
    setTime(timeRef.current)
  }
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-white/[0.09] bg-[#0B1022] shadow-[0_48px_140px_-40px_rgba(224,106,50,0.25)]">
      ...
      <div ref={stageRef} className="relative aspect-video w-full" style={darkSceneVars}>
        ...
        {chapter.Comp && chapter.caption && (
          <div className="pointer-events-none absolute inset-x-0 bottom-10 hidden justify-center sm:flex">
            <div className="mx-6 max-w-xl rounded-xl border border-white/[0.08] bg-[#0B1022]/85 px-4 py-2.5 text-center backdrop-blur"
              style={{ opacity: reduced ? 1 : Math.min(easeOut(win(local, 0.02, 0.12)), 1 - win(local, 0.92, 1)) }}>
      ...
      <div className="flex items-center gap-3 border-t border-white/[0.07] bg-[#0D1226] px-4 py-3">
        <button onClick={toggle} disabled={reduced}
          className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white/[0.08] text-white transition-colors hover:bg-[#E06A32] disabled:opacity-40"
