    ...
    <>
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-md" showCloseButton>
          ...
          {uploadState === 'success' && result && (
            <div className="space-y-3 py-2">
              <div className="flex items-center gap-3">
                <CheckCircle2 className="h-8 w-8 text-pos shrink-0" />
                <div>
              ...
              {result.warnings.length > 0 && (
                <div className="rounded-lg bg-shell border border-line p-3 space-y-1.5">
                  <div className="flex items-center gap-1.5">
                    <AlertTriangle className="h-3.5 w-3.5 text-tx-2 shrink-0" />
                    <p className="text-xs font-semibold text-tx-2">{result.warnings.length} warning{result.warnings.length > 1 ? 's' : ''}</p>
                  </div>
              ...
              {result.errors.length > 0 && (
                <div className="rounded-lg bg-flare-wash border border-line-strong p-3 space-y-1.5">
                  <div className="flex items-center gap-1.5">
                    <XCircle className="h-3.5 w-3.5 text-tx-flare shrink-0" />
                    <p className="text-xs font-semibold text-tx-flare">{result.errors.length} row{result.errors.length > 1 ? 's' : ''} skipped</p>
                  </div>
                  <ul className="text-xs text-tx-flare space-y-0.5 list-disc list-inside">
          ...
          {/* Error state */}
          {uploadState === 'error' && (
            <div className="flex items-start gap-3 rounded-lg bg-flare-wash border border-line-strong p-4">
              <XCircle className="h-5 w-5 text-tx-flare mt-0.5 shrink-0" />
              <div>
                <p className="text-sm font-medium text-tx-flare">Upload failed</p>
                <p className="text-xs text-tx-flare mt-0.5">{errorMsg}</p>
              </div>
            </div>
          )}
